...
describe('SetInactiveStatusCase', () => {

    let sut: SetInactiveStatusCase
    ...
    describe('Target user validation and domain invariants (PHASE 3)', () => {
        ...
        it('should throw a domain error if trying to pass the project owner to inactive status', async () => {
            const actor = UserMother.reconstituteDefault()
            const project = ProjectMother.reconstituteDefault({ ownerId: actor.id })
            ...
            await expect(sut.execute(input)).rejects.toThrow(MemberErrorFactory.memberOwnerLocked().message)
            expect(mockMemberRepository.save).not.toHaveBeenCalled()
        })

        it('should throw a domain error if the target member has active pending tasks in the project', async () => {
            const actor = UserMother.reconstituteDefault()
            const project = ProjectMother.reconstituteDefault({ ownerId: actor.id })
            const targetUser = UserMother.createDefault()
            const targetMember = MemberMother.reconstituteDefault({ projectId: project.id, userId: targetUser.id })
            const actingMember = MemberMother.reconstituteDefault({ projectId: project.id, userId: actor.id, role: MemberRoleVo.create('admin') })
            ...
            mockTaskRepository.hasUserTaskPendings.mockResolvedValue(true)

            const input = { actorId: actor.publicId.value, projectId: project.publicId.value, targetMemberId: targetMember.publicId.value }

            await expect(sut.execute(input)).rejects.toThrow(MemberErrorFactory.memberActiveTasksInactivate().message)
            expect(mockMemberRepository.save).not.toHaveBeenCalled()
        })
    })
    ...
    describe('Orchestration, aggregate persistence and data integrity (PHASE 4)', () => {
        ...
        it('should successfully update member status to inactive and return primitive DTO when executed by a legitimate project ADMIN', async () => {
            const actor = UserMother.reconstituteDefault()
            const project = ProjectMother.reconstituteDefault() 
            const actingMember = MemberMother.reconstituteDefault({ projectId: project.id, userId: actor.id, role: MemberRoleVo.create('admin') })
            
            const targetMember = MemberMother.reconstituteDefault({ projectId: project.id, status: MemberStatusVo.create('active') })
            const targetUser = UserMother.createDefault()
