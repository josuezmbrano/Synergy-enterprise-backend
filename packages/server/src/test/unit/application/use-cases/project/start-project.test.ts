...
describe('StartProjectCase', () => {

    let sut: StartProjectCase
    ...
    beforeEach(() => {
        ...
        sut = new StartProjectCase(mockProjectRepository, mockUserRepository, mockTaskRepository, mockMemberRepository)
    })
    ...
    describe('Project existence, membership linkage and access control validation (PHASE 2)', () => {
        ...
        it('should throw a ProjectDomain project not found error if membership status is not active', async () => {
            const user = UserMother.reconstituteDefault()
            const project = ProjectMother.reconstituteDefault()
            ...
            await expect(sut.execute(input)).rejects.toThrow(ProjectErrorFactory.projectNotFound().message)
            expect(mockMemberRepository.findProjectMember).toHaveBeenCalledWith(expect.any(ProjectIdVo), expect.any(UserIdVo))
            expect(mockTaskRepository.hasTasks).not.toHaveBeenCalled()
            expect(spyOnAccess).toHaveBeenCalled()
        })

        it('should throw a ProjectDomain project not owner or admin if update is being done without proper operational privilege', async () => {
            const user = UserMother.reconstituteDefault()
            const project = ProjectMother.reconstituteDefault()
            ...
            await expect(sut.execute(input)).rejects.toThrow(ProjectErrorFactory.projectNotOwnerOrAdmin().message)
            expect(mockTaskRepository.hasTasks).not.toHaveBeenCalled()
        })
    })

    describe('Tasks and members required validation control to update (PHASE 3)', () => {

        it('should throw a ProjectDomain project tasks required error if the update is attempted while there are no tasks created', async () => {
            const user = UserMother.reconstituteDefault()
            ...
            mockProjectRepository.findByPublicId.mockResolvedValue(project)
            mockMemberRepository.findProjectMember.mockResolvedValue(member)
            mockMemberRepository.hasTeamMembers.mockResolvedValue(true)
            mockTaskRepository.hasTasks.mockResolvedValue(false)

            const input = { actorId: user.publicId.value, projectId: project.publicId.value }

            await expect(sut.execute(input)).rejects.toThrow(ProjectErrorFactory.projectTasksRequired().message)
            expect(mockMemberRepository.hasTeamMembers).not.toHaveBeenCalled()
        })

        it('should throw a ProjectDomain project members required error if the update is attempted while there are no members associated to the project', async () => {
            const user = UserMother.reconstituteDefault()
            const project = ProjectMother.createWithPersonalizedProps({ ownerId: user.id })
            const member = MemberMother.reconstituteAdmin()
            mockUserRepository.findByPublicId.mockResolvedValue(user)
            mockProjectRepository.findByPublicId.mockResolvedValue(project)
            mockMemberRepository.findProjectMember.mockResolvedValue(member)
            mockMemberRepository.hasTeamMembers.mockResolvedValue(false)
            mockTaskRepository.hasTasks.mockResolvedValue(true)

            const input = { actorId: user.publicId.value, projectId: project.publicId.value }

            await expect(sut.execute(input)).rejects.toThrow(ProjectErrorFactory.projectMembersRequired().message)
        })
    })

    describe('Status update persistence and DTO return orchestration (PHASE 4)', () => {
        ...
        it('should update, persist changes and return expected DTO format', async () => {
            ...
            const input = { actorId: user.publicId.value, projectId: project.publicId.value }
            const spyOnMoveStatus = vi.spyOn(project, 'moveToInProgress')

            const results = await sut.execute(input)

            expect(spyOnMoveStatus).toHaveBeenCalled()
