...
describe('CompleteProjectCase', () => {

    let sut: CompleteProjectCase
    ...
    beforeEach(() => {
        ...
        sut = new CompleteProjectCase(mockProjectRepository, mockUserRepository, mockTaskRepository, mockMemberRepository)
    })
    ...
    describe('Project existence, membership linkage and access control validation (PHASE 2)', () => {
        ...
        it('should throw a ProjectDomain project not found error if membership status is not active', async () => {
            ...
            expect(mockTaskRepository.hasPendingTasks).not.toHaveBeenCalled()
        })
    ...
    describe('Project related tasks required validation control to update (PHASE 3)', () => {

        it('should throw a ProjectDomain project completion pending tasks error if the update is attempted while tasks are not completed', async () => {
            const user = UserMother.reconstituteDefault()
            const project = ProjectMother.createWithPersonalizedProps({ ownerId: user.id, status: ProjectStatusVo.create('in_progress') })
            const member = MemberMother.reconstituteAdmin()
            mockUserRepository.findByPublicId.mockResolvedValue(user)
            mockProjectRepository.findByPublicId.mockResolvedValue(project)
            mockMemberRepository.findProjectMember.mockResolvedValue(member)
            mockTaskRepository.hasPendingTasks.mockResolvedValue(true)

            const input = { actorId: user.publicId.value, projectId: project.publicId.value }

            await expect(sut.execute(input)).rejects.toThrow(ProjectErrorFactory.projectCompletionPendingTasks().message)
            expect(mockTaskRepository.hasPendingTasks).toHaveBeenCalledWith(project.id)
    ...
    describe('Status update persistence and DTO return orchestration (PHASE 4)', () => {
        ...
        it('should throw a CommonDomain error if owner public id is not present on updated project records', async () => {
            ...
            mockProjectRepository.findByPublicId.mockResolvedValue(project)
            mockMemberRepository.findProjectMember.mockResolvedValue(member)
            mockTaskRepository.hasPendingTasks.mockResolvedValue(false)
        ...
        it('should update, persist changes and return expected DTO format', async () => {
            const user = UserMother.reconstituteDefault()
            const project = ProjectMother.reconstituteDefault({ ownerId: user.id, status: ProjectStatusVo.create('in_progress') })
            const member = MemberMother.reconstituteAdmin()
            mockUserRepository.findByPublicId.mockResolvedValue(user)
            mockProjectRepository.findByPublicId.mockResolvedValue(project)
            mockMemberRepository.findProjectMember.mockResolvedValue(member)
            ...
            const input = { actorId: user.publicId.value, projectId: project.publicId.value }
            const spyOnMoveStatus = vi.spyOn(project, 'moveToCompleted')
            ...
            expect(results.completedAt).not.toBeNull()
        })
    })
