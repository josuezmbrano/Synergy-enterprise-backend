...
describe('UpdateTaskDuedateCase', () => {

    let sut: UpdateTaskDuedateCase
    ...
    describe('Orchestration, aggregate persistence and data integrity (PHASE 4)', () => {
        ...
        it('should successfully update the due date, save into the repository and return the mapped DTO when permissions are valid', async () => {
            const actor = UserMother.reconstituteDefault()
            ...
            const task = TaskMother.reconstitutePersonalized(creatorMember.publicId, undefined, {
                projectId: project.id,
                dueDate: DateVo.create('2026-06-01T00:00:00.000Z')
            })
            ...
            const targetNewDate = '2026-12-25T12:00:00.000Z'
            const input = {
                actorId: actor.publicId.value,
                taskId: task.publicId.value,
                dueDate: targetNewDate
            }
            ...
            expect(result.dueDate).toBe(new Date(targetNewDate).toISOString())
        })
    })
