...
describe('SetHighPriorityCase - Integration Tests', () => {
    let useCase: SetHighPriorityCase;
    ...
    beforeEach(async () => {
        ...
        useCase = containerDI.modules.task.useCases.setHighPriorityUseCase
    });
    ...
    describe('Set High Priority - Happy Path', () => {

        it('should successfully update task priority to HIGH', async () => {
            ...
            // Persist the targeted task aggregate instance explicitly configured with a baseline MEDIUM priority and active state
            const task = await seedTaskRandom(prisma, projectPrimitives.id, creatorMember.toPrimitives().id, creatorMember.toPrimitives().id, {
