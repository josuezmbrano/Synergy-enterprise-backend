import { UpdateTaskAssigneeOutput } from 'application/dtos/task/update-task-assignee.dto.js';
import { UpdateTaskAssigneeInput } from '@project/common/schemas/task.schema.js'
...
export class UpdateAssigneeCase implements BaseUseCase<UpdateTaskAssigneeInput, UpdateTaskAssigneeOutput> {
    ...
    async execute(input: UpdateTaskAssigneeInput): Promise<UpdateTaskAssigneeOutput> {

        // INSTANTIATE VOs
        const actingUserPublicId = UserIdVo.fromId(input.actorId)
        const taskId = TaskIdVo.fromId(input.taskId)
        const assigneeMemberId = MemberIdVo.fromId(input.assigneeId)
        ...
        // VALIDATE OWNERSHIP OR CREATOR PRIVILEGE
        ...
        // VALIDATE ASSIGNEE MEMBERSHIP TO PROJECT
        const targetMember = await this.memberRepository.findByPublicId(assigneeMemberId)
        ...
        task.assignTo(targetMember.id)


        // PERSIST TO REPOSITORY AND OUTPUT PRIMITIVES TO CLIENT
        const taskUpdated = await this.taskRepository.save(task)


        // RETRIEVE NEW UPDATED ASSIGNED USER PUBLIC ID AND CREATOR PUBLIC ID
        const assignedToPublicId = taskUpdated.assignedToPublicId
        const creatorPublicId = taskUpdated.creatorPublicId

        if (!assignedToPublicId || !creatorPublicId) {
    ...
    private mapToOutput(task: TaskEntityClass, projectPublicId: ProjectIdVo, creatorPublicId: MemberIdVo, assignedToPublicId: MemberIdVo | undefined): UpdateTaskAssigneeOutput {
