import { SetDoingStatusOutput } from 'application/dtos/task/status-dtos/set-doing-status-dto.js';
...
export class SetDoingStatusCase implements BaseUseCase<SetDoingStatusInput, SetDoingStatusOutput> {
    ...
    async execute(input: SetDoingStatusInput): Promise<SetDoingStatusOutput> {
        ...
        // VALIDATE TASK ASSIGNED USER WIP LIMIT AND CURRENT STATUS AS MEMBER
        ...
        // VALIDATE TASK ASSIGNED USER WIP LIMIT
        const activeTasksNumber = await this.taskRepository.countActiveTaskByUser(project.id, targetMemberAssigned.id)
        project.ensureUserHasWipLimit(activeTasksNumber)


        // CALL THE SPECIFIC METHOD WITH THE ASSIGNED USER
        // TO INTERNALLY VALIDATE THAT IS THE SAME ASSIGNED USER
        task.moveToDoing(targetMemberAssigned.id)
    ...
    private mapToOutput(task: TaskEntityClass, projectPublicId: ProjectIdVo, creatorPublicId: MemberIdVo, assignedToPublicId: MemberIdVo | undefined): SetDoingStatusOutput {
