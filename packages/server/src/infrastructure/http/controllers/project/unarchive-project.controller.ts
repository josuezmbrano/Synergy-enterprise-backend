import { UnarchiveProjectInput } from '@project/common/schemas/project.schema.js';
import { UnarchiveProjectCase } from 'application/use-cases/project/unarchive-project.usecase.js';
import type { Request, Response, NextFunction } from 'express-serve-static-core';

export class UnarchiveProjectController {
    constructor(private readonly unarchiveProjectUseCase: UnarchiveProjectCase) { }

    execute = async (req: Request, res: Response, next: NextFunction): Promise<void> => {

        try {
             
            // Retrieve the actor id from the auth payload
            const actorId = req.user.sub

            // Retrieve the project id from the request parameters
            const projectId = req.params.projectId as string

            const input: UnarchiveProjectInput = {
                actorId,
                projectId
            }

            const result = await this.unarchiveProjectUseCase.execute(input)

            res.status(200).json({
                status: 'success',
                data: result
            })

        } catch (error) {

            next(error) // the error goes up to the GlobalErrorMiddleware implementation
        }
    }

}